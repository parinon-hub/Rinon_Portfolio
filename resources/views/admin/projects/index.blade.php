<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
                <p class="text-xs font-medium text-accent tracking-widest uppercase mb-3">Portfolio</p>
                <h2 class="font-display font-bold text-4xl md:text-5xl text-zinc-900 dark:text-white leading-tight">
                    {{ __('Manage Projects') }}
                </h2>
            </div>
            <a href="{{ route('projects.create') }}" class="btn-primary">
                Add Project <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
            </a>
        </div>
    </x-slot>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="border-b border-zinc-100 dark:border-zinc-800">
                        <th class="py-4 font-display font-bold text-zinc-900 dark:text-white uppercase tracking-wider text-xs">Project</th>
                ...
                <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800 text-sm">
                    ...
                    <tr class="group hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                        ...
                        <td class="py-4">
                            <span class="text-xs bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 px-3 py-1 rounded-full">{{ $project->category }}</span>
                        </td>
                        <td class="py-4 text-right">
                            <div class="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                <a href="{{ route('projects.edit', $project) }}" class="text-zinc-500 hover:text-accent transition-colors">
