...
<meta name="description" content="{{ $pageContents['meta_description'] ?? 'Patrick Riñon — Information Systems Student and Aspiring Professional.' }}">
<meta name="keywords" content="Patrick Riñon, portfolio, Information Systems, Philippines">
<meta name="author" content="Patrick Riñon">
<title>@yield('title', $pageContents['site_title'] ?? 'Patrick Riñon — Portfolio')</title>
...
<header class="fixed inset-x-0 top-0 z-50 transition-all duration-300"
  ...
  <nav class="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between" aria-label="Main navigation">
    ...
    <a href="{{ route('home') }}#hero" class="font-display font-bold text-xl tracking-tight relative z-10">
      <span class="text-zinc-900 dark:text-white">{{ substr($pageContents['site_name'] ?? 'patrick', 0, -3) }}</span><span class="text-accent">{{ substr($pageContents['site_name'] ?? 'patrick', -3) }}</span>
    </a>

    <ul class="hidden md:flex items-center gap-8 text-sm" role="list">
      <li><a href="{{ route('home') }}#services" class="nl text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition-colors" :class="s==='services'?'on !text-zinc-900 dark:!text-white':''">Services</a></li>
      <li><a href="{{ route('home') }}#work"     class="nl text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition-colors" :class="s==='work'?'on !text-zinc-900 dark:!text-white':''">Work</a></li>
      <li><a href="{{ route('home') }}#about"   class="nl text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition-colors" :class="s==='about'?'on !text-zinc-900 dark:!text-white':''">About</a></li>
      <li><a href="{{ route('home') }}#reviews" class="nl text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition-colors" :class="s==='reviews'?'on !text-zinc-900 dark:!text-white':''">Reviews</a></li>
      <li><a href="{{ route('home') }}#blog"    class="nl text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition-colors" :class="s==='blog'?'on !text-zinc-900 dark:!text-white':''">Blog</a></li>
      <li><a href="{{ route('home') }}#contact" class="nl text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition-colors" :class="s==='contact'?'on !text-zinc-900 dark:!text-white':''">Contact</a></li>
    </ul>
    ...
    <div class="flex items-center gap-3">
      ...
      @auth
        <a href="{{ route('dashboard') }}" class="text-xs font-medium text-zinc-500 hover:text-accent">Admin</a>
      @else
        <a href="{{ route('login') }}" class="text-xs font-medium text-zinc-500 hover:text-accent">Login</a>
      @endauth
      ...
      <a href="#contact" class="hidden md:inline-flex items-center gap-2 shimmer bg-accent text-white text-sm font-medium px-5 py-2 rounded-full hover:bg-accent-light transition-colors">
        Hire me <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
      </a>
      <!-- hamburger -->
...
<footer class="border-t border-zinc-100 dark:border-zinc-900">
  <div class="max-w-6xl mx-auto px-6 py-8 flex flex-col sm:flex-row items-center justify-between gap-4">
    <p class="text-sm text-zinc-400">© <span id="yr"></span> {{ $pageContents['site_name'] ?? 'Patrick' }}. All rights reserved.</p>
    <p class="text-xs text-zinc-500">Built with Laravel, Tailwind CSS &amp; Alpine.js</p>
  </div>
</footer>
